...
echo "generator-baseline  pid=$!  → $DOCKER_HOST:11883  rate=81420  ws=:8769"
...
# This ensures the comparison is fair (same input, different Telegraf pipeline).
nohup "$STRESS_BIN" \
    --host "$DOCKER_HOST" \
    --port 11884 \
    --template "$STRESS_TPL" \
...
echo "generator-filtered  pid=$!  → $DOCKER_HOST:11884  rate=81420  ws=:8779"
